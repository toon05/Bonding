...
public class RollGenerator : IRollGenerator
{
    ...
    public string GeneratePrompt(int gl)
    {
        ...
        int level = 0;
        if (_growthLevel <= 5) level = 0;
        if (_growthLevel <= 15) level = 1;
        if (_growthLevel > 15) level = 2;
        prompt += _growthRateTexts[level] + "\n";
        prompt += $"私のことを「{_playerName}」とよび、\nそのパートナーのことを「{_partnerName}」と呼びます。";
        prompt += "以下はあなたの特徴です。これに従って30文字以内で喋ってください。なお、文頭に【指示】と表記されている場合には、それがあなたの話す内容ですので、その内容をロボットの話し言葉に書き換えて、ユーザーに喋りかけてください。";
